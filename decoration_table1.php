...
<body>
    ...
    <div class="left-panel">
        ...
        <div class="charts">
       <center> <h2>Decoration Details</h2></center>
    <table>
        <tr>
            <th>Name</th>
            <th>Age</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["name"]."</td><td>".$row["age"]."</td><td>".$row["gender"]."</td><td>".$row["rollno"]."</td><td>".$row["class"]."</td><td>".$row["section"]."</td><td>".$row["event"]."</td><td><a href='decoration_table_edit1.php?rollno=".$row["rollno"]."'>Edit</a> | <a href='decoration_table_delete1.php?rollno=".$row["rollno"]."'>Delete</a></td></tr>";
...
if(isset($_POST['rollno'], $_POST['name'], $_POST['age'], $_POST['gender'], $_POST['class'],$_POST['section'],$_POST['event'])) {
    ...
    $stmt = $conn->prepare("UPDATE `decoration1` SET `name` = ?, `age` = ?, `gender`=?,`class`=?,`section`=?,`event`=? WHERE `rollno` = ?");
